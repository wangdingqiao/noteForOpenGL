...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"Demo of blending(complete transparency)", NULL, NULL);
	if (!window)
	{
	...
	GLuint transparentTextId = TextureHelper::load2DTexture(
		"../../resources/textures/grass.png", GL_RGBA, GL_RGBA, SOIL_LOAD_RGBA, true);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (std::vector<glm::vec3>::const_iterator it = vegetation.begin(); 
			vegetation.end() != it; ++it)
		{
			model = glm::mat4();
