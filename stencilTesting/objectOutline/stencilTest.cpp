...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"Demo of outline(using stencil buffer)", NULL, NULL);
	if (!window)
	{
	...
	// 平面的顶点属性数据 顶点位置 纹理坐标(设置的值大于1.0用于重复)
	...
	Shader outlineShader("singleColor.vertex", "singleColor.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		outlineShader.use();
		glUniformMatrix4fv(glGetUniformLocation(outlineShader.programId, "projection"),
			1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(glGetUniformLocation(outlineShader.programId, "view"),
			1, GL_FALSE, glm::value_ptr(view));
		...
		glUniformMatrix4fv(glGetUniformLocation(outlineShader.programId, "model"),
			1, GL_FALSE, glm::value_ptr(model));
