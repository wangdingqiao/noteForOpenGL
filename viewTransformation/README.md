...
1. 相机绕着+y轴，旋转+90度，此时相机镜头朝向立方体的右侧面，实现目的。注意这时立方体并没有转动。完成这一旋转的矩阵记作$R_{y}(\frac{\pi}{2})$
2. 相机不动，让立方体绕着+y轴，旋转-90度，此时也能实现同样的目的。注意这时相机没有转动。完整这一旋转的矩阵记作$R_{y}(-\frac{\pi}{2})$
