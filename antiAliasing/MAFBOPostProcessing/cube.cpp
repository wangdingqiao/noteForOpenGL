...
bool prepareIntermediateFBO(GLuint& textId, GLuint& fboId);
// 定义程序常量
...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"Demo of multiSampled FBO(postProcessing)", NULL, NULL);
	if (!window)
	{
	...
	if (!prepareIntermediateFBO(screenTextId, intermediateFBOId))
	{
...
bool prepareMSFBO(const int samplesCnt, GLuint& textId, GLuint& fboId)
{
	...
	// 附加 color attachment
	textId = TextureHelper::makeMAAttachmentTexture(samplesCnt, GL_RGB); // 创建FBO中的多采样纹理
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, textId, 0);
...
bool prepareIntermediateFBO(GLuint& colorTextId, GLuint& fboId)
{
	glGenFramebuffers(1, &fboId);
	glBindFramebuffer(GL_FRAMEBUFFER, fboId);
	// 附加 color attachment
