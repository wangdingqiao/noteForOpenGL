...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"Demo of instancing( without instance array)", NULL, NULL);
	if (!window)
	{
    ...
    // Section5 向shader传送位移量数据
	shader.use();
	for (GLint i = 0; i < 100; ++i)
	{
		std::ostringstream indexAsStr;
		indexAsStr << i;
		std::string uniformStr("offsets[");
		uniformStr += indexAsStr.str()+"]";
		glUniform2f(glGetUniformLocation(shader.programId, uniformStr.c_str()),
