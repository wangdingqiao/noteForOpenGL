...
int main(int argc, char** argv)
{
	...
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT,
		"Demo of Using UBO in shader(layout=std140)", NULL, NULL);
	if (!window)
	{
	...
	// Section5 使用一个复杂的UBO作为例子
	redShaderIndex = glGetUniformBlockIndex(redShader.programId, "ExampleBlock");
	greeShaderIndex = glGetUniformBlockIndex(greenShader.programId, "ExampleBlock");
	blueShaderIndex = glGetUniformBlockIndex(blueShader.programId, "ExampleBlock");
	yellowShaderIndex = glGetUniformBlockIndex(yellowShader.programId, "ExampleBlock");
	// step2 设置shader中 uniform buffer 的索引到指定绑定点
