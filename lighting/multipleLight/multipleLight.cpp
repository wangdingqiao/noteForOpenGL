...
void setupLights(Shader& shader, glm::vec3* PointLightPositions, int pointLightCnt)
{
	...
	glUniform3f(glGetUniformLocation(shader.programId, "dirLight.ambient"), 0.0f, 0.1f, 0.4f);
	glUniform3f(glGetUniformLocation(shader.programId, "dirLight.diffuse"), 0.0f, 0.2f, 0.8f);
	glUniform3f(glGetUniformLocation(shader.programId, "dirLight.specular"), 0.0f, 1.0f, 1.0f);
	glUniform3f(glGetUniformLocation(shader.programId, "dirLight.direction"), lampDir.x, lampDir.y, lampDir.z); // 方向光源
	// 设置点光源
	for (int i = 0; i < pointLightCnt; ++i)
	{
		...
		GLint lightAmbientLoc = glGetUniformLocation(shader.programId, (lightName + ".ambient").c_str());
		GLint lightDiffuseLoc = glGetUniformLocation(shader.programId, (lightName + ".diffuse").c_str());
		GLint lightSpecularLoc = glGetUniformLocation(shader.programId, (lightName + ".specular").c_str());
		GLint lightPosLoc = glGetUniformLocation(shader.programId, (lightName + ".position").c_str());
		GLint attConstant = glGetUniformLocation(shader.programId, (lightName + ".constant").c_str());
		GLint attLinear = glGetUniformLocation(shader.programId, (lightName + ".linear").c_str());
		GLint attQuadratic = glGetUniformLocation(shader.programId, (lightName + ".quadratic").c_str());
	...
	// 设置FlashLight光源
	glUniform3f(glGetUniformLocation(shader.programId, "spotLight.ambient"), 0.0f, 0.1f, 0.4f);
	glUniform3f(glGetUniformLocation(shader.programId, "spotLight.diffuse"), 0.0f, 0.2f, 0.8f);
	glUniform3f(glGetUniformLocation(shader.programId, "spotLight.specular"), 0.0f, 1.0f, 1.0f);
	...
	glUniform3f(glGetUniformLocation(shader.programId, "spotLight.direction"), camera.forward.x, camera.forward.y, camera.forward.z);
	glUniform1f(glGetUniformLocation(shader.programId, "spotLight.cutoff"), cos(glm::radians(12.5f)));
	glUniform1f(glGetUniformLocation(shader.programId, "spotLight.outerCutoff"), cos(glm::radians(17.5f)));
	// 设置衰减系数
	glUniform1f(glGetUniformLocation(shader.programId, "spotLight.constant"), 1.0f);
	glUniform1f(glGetUniformLocation(shader.programId, "spotLight.linear"), 0.09f);
	glUniform1f(glGetUniformLocation(shader.programId, "spotLight.quadratic"), 0.032f);
}
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (key >= 0 && key < 1024)
